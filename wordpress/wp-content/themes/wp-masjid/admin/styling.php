...
?>
    ...
    <script>
		    	jQuery(document).ready(function($){
					...
					$('.shares').click(function() {
						...
						var button = $(this);
						wp.media.editor.send.attachment = function(props, attachment) {
						jQuery(".custom_share").attr('src', attachment.url);
						$(button).prev().val(attachment.url);
						wp.media.editor.send.attachment = send_attachment_bkp;
						$('.custom_share, .sak').show();
						}
						wp.media.editor.open(button);
						return false;  
					});
	...
	<tr>
		<td class="tl"><label>Gambar Default Share</label></td>
    	<td>
	
         	<div class="pekol">
		    	<?php if (get_option('share')) { ?>
		    		<span class="fak bl">x</span>
		    	<?php } else { ?>
		    		<span class="fak no">x</span>
		    	<?php } ?>
		    	<img class="custom_share" src="<?php echo get_option('share'); ?>"/><br/>
			</div>
	    	<div class="pekol">
			    <input class="custom_media_url" id="share" type="hidden" name="share" value="<?php echo get_option('share'); ?>"> <a href="#" class="button shares custom_media_upload">Upload Gambar</a><br/>
			</div>
	
        	<span class="description"><strong>PENTING</strong> : Untuk gambar default untuk share media sosial dan format jpg, gif, atau png</span>
			<br/><br/>
    	</td>
	</tr>
