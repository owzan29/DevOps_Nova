    	...
    	<div class="weleft">
        	<?php if (have_posts()): ?>
				...
				<div class="blog-content clear">
				    <h2><?php the_title(); ?></h2>
					<div class="met">
						<strong>Terbit</strong> <i class="far fa-clock"></i> <?php printf(__('%s', 'weesata'), get_the_time('j F Y')); ?> | <strong>Oleh</strong> : <?php the_author(); ?> | <strong>Kategori</strong> : <?php $terms = get_the_terms( $post->ID , 'kat-tausiyah' );
								if ( $terms != null ){
									foreach( $terms as $term ) {
									    $term_link = get_term_link( $term, 'kat-tausiyah' );
				    ...
				    <?php the_content(); ?>
					<div class="post-tags"><?php $terms = get_the_terms( $post->ID , 'topik' );
								if ( $terms != null ){
									foreach( $terms as $term ) {
									    $term_link = get_term_link( $term, 'topik' );
										    echo '<a href="'.$term_link.'">'.$term->name.'</a>';
											unset($term); 
										} 
									} 
        	...
        	<?php endif; ?>
			    ...
			    <?php query_posts('post_type=tausiyah&showposts=3'); ?>
	                <div class="blog-content">
		            	<div class="clear">
		        	 		<h3><span>Tausiyah Lainnya</span></h3>
		            	</div>
		        	</div>
